...
import { DEMAND_PROFILES, DEMAND_SEASONAL, DEMAND_WEATHER, UNIT_ENERGY_KWH } from "./config";
...
export function weatherDemandMultiplier(tempC: number): number {
  const heating =
    DEMAND_WEATHER.heatingPerC * Math.max(0, DEMAND_WEATHER.heatingThresholdC - tempC);
  const cooling =
    DEMAND_WEATHER.coolingPerC * Math.max(0, tempC - DEMAND_WEATHER.coolingThresholdC);
...
export function cityDemandMwAtHour(
  ...
  const firmKw =
    ((firms * UNIT_ENERGY_KWH.firm[dayType]) / 24) * (DEMAND_PROFILES.firm[dayType][hour] ?? 0);
...
export function cityDemandDayMw(
  ...
  for (let hour = 0; hour < 24; hour++) {
    result.push(
      quantize01(cityDemandMwAtHour(households, firms, dayType, hour, month, tempC[hour] ?? 15)),
    );
  }
  return result;
}
