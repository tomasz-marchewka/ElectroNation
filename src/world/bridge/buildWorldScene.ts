...
function buildLines(
  ...
  report: TurnReport | null,
  labels: WorldLabel[],
): { lines: WorldLine[]; hottest: { ratio: number; text: string; hex: HexRef } | null } {
  ...
  let hottest: { ratio: number; text: string; hex: HexRef } | null = null;
  ...
  const lines = state.lines.map((line): WorldLine => {
    ...
    const segments: WorldSegment[] = measured.map((segment) => {
      ...
      if (ratio >= LOAD_OVER_RATIO && segment.usedMw > IDLE_FLOW_MW) {
        const at = path[Math.floor((segment.fromIndex + segment.toIndex) / 2)];
        if (at && (hottest === null || ratio > hottest.ratio)) {
          hottest = {
            ratio,
...
export function buildWorldScene(
  ...
  const { lines, hottest } = buildLines(state, report, labels);
  ...
  // Weather the objects felt: the SHOWN day's truth (the override, if any,
  // only repaints the sky — the numbers of the turn stay the engine's).
  const truth = shown.truth;
  ...
  const plants: WorldPlant[] = state.plants.map((plant) => {
    ...
    labels.push({
      key: `${plant.id}:label`,
      hex: ref(plant.hex),
      text: plantLabel(plant.name, plant.tech, plantOrderMw(plant), plant.capacityMw),
      tone: "default",
      kind: "object",
      muted: false,
      priority: 2,
    ...
    return {
      ...
      outputMw,
      usedMw,
      dumpMw: shown.report ? Math.max(0, outputMw - usedMw) : 0,
  ...
  const farms: WorldFarm[] = state.farms.map((farm) => {
    ...
    const windSeries = truth.weather.windMs[farm.windClass];
    ...
    for (let h = 0; h < HOURS_PER_TURN; h++) {
      potentialMw += farmPowerMwAtHour(farm, truth.weather, startHour + h);
    }
    potentialMw /= HOURS_PER_TURN;
    const producedMw = farm.enabled ? (sourceRows.get(farm.id)?.offeredMw ?? potentialMw) : 0;
    const usedMw = sourceRows.get(farm.id)?.usedMw ?? 0;
    const cap =
  ...
  if (hottest !== null) {
    const hot: { ratio: number; text: string; hex: HexRef } = hottest;
    labels.push({
      key: "overload",
      hex: hot.hex,
      text: hot.text,
