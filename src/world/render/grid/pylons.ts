...
import { GLASS_TINT, Truss, panel, square, type Point } from "./lattice";
...
export const PYLON_SPECS: Record<LineType, PylonSpec> = {
  ...
  mv: spec(
    ...
    [0, 0.2, 0.4, 0.58, 0.78],
    [
      { y: 0.58, reach: 0.35, hangs: [0.17, 0.32] },
      { y: 0.78, reach: 0.2, hangs: [0.17] },
    ],
  ...
  hv: spec(
    ...
    [
      { y: 0.6, reach: 0.25, hangs: [0.23] },
      { y: 0.82, reach: 0.31, hangs: [0.29] },
      { y: 1.02, reach: 0.25, hangs: [0.23] },
    ],
    [
...
function arm(truss: Truss, s: PylonSpec, level: ArmLevel, side: 1 | -1): void {
  ...
  for (const hang of level.hangs) {
    ...
    truss.bar(at, { x: at.x, y: level.y - s.insulator, z: 0 }, s.member.brace * 0.8, GLASS_TINT);
  }
}
...
export function nearPylonGeometry(s: PylonSpec): THREE.BufferGeometry {
 ...
 */
export function portalGeometry(s: PylonSpec): THREE.BufferGeometry {
  const p = s.portal;
  const truss = new Truss();
...
export function buildPylonGeometries(): Record<LineType, PylonGeometries> {
  ...
  for (const type of LINE_TYPES) {
    ...
    out[type] = { near: nearPylonGeometry(s), far: farPylonGeometry(s), portal: portalGeometry(s) };
  }
  return out;
}
